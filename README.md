...
_**const variables**_ ex: const int number; 
  <hr>.
_**Const pionter**_
...
On the other hand, if you just want the address stored in the pointer itself to be const, then you have to put const after the *:<br>
int x;
...
_**Use & to declare a reference to a type**_
...
<br>std::string mrSamberg("Andy");
std::string& theBoss = mrSamberg<hr>
